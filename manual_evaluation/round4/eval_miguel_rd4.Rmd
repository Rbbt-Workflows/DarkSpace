...
#### Estimating size to curate

```{r summ_stat}
median(dsp_scored_rd4$Relevance)
mean(dsp_scored_rd4$Relevance)
```
We assume a % of true positives of around 90% for those publications with a relevance score >6.5 (`r nrow(dsp_scored_rd4[Relevance>6.5])` publications, `r round(nrow(dsp_scored_rd4[Relevance>6.5])*.9)` estimated true positives). Considering also those below that score, and estimating a true positive rate of 40%, that adds `r round(nrow(dsp_scored_rd4[Relevance<6.5])*.4)` potentially positive publications below 6.5. Total number of potentially positive publications is `r round(nrow(dsp_scored_rd4[Relevance>6.5])*0.9 + nrow(dsp_scored_rd4[Relevance<=6.5])*.4)`.